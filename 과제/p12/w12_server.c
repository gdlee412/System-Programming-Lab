...
int main (int argc, char *argv[]) {
    ...
    char buffer[MAXLINE], username[11];
    //ssize_t user_size;
    ...
    while (1) {
        ...
        for (int fd = 0; fd < fd_max + 1; fd++) {
			...
			printf ("got %lu bytes from user .\n", num_bytes);
			sprintf(username, "%lu:", num_bytes);
			for(int j = 0; j < fd_max + 1; j++) {
				if(FD_ISSET(j, &read_set)) {
					if(j != listening_fd && j != fd) {
						write(j, username, strlen(username));
						write(j, buffer, (size_t) num_bytes);
		       			}
				}
			}
		    }
