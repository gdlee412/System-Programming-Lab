...
int main (int argc, char* argv[]) {
        ...
        if((listening_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {                printf("socket() failed.\n");
		exit(2);
        }

        memset((char *)&socket_address, 0, sizeof(socket_address)); // initializes socket_address fields to zero
        socket_address.sin_family = AF_INET; // sets address family to IPv4         socket_address.sin_addr.s_addr = htonl(INADDR_ANY); // copies ip address
